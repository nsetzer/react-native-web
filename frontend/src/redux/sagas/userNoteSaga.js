...
function* _userNoteGetContent(action) {
  ...
  try {
    ...
    // e.g. 404
    console.log(e.response.status === 404 ? 'true' : 'false')
    if (e.response.status === 404) {
      console.log()
      // this will occur when the user creates a new note from the ui
      content = {loading: false, loaded: true, error: null, saving:false, deleting: false, text: ''}
      yield put({type: USER_NOTE_SET_CONTENT, uid: action.uid, content})
    } else {
